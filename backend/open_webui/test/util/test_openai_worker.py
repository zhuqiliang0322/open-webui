...
def test_extract_openclaw_worker_prompt_reads_chat_completion_messages():
    ...
    assert prompt == 'that requires multiple agents'


def test_extract_openclaw_worker_prompt_uses_latest_responses_user_message():
    prompt = openai.extract_openclaw_worker_prompt(
        {
            'input': [
                {
                    'type': 'message',
                    'role': 'user',
                    'content': [{'type': 'input_text', 'text': '/heavy older request'}],
                },
                {
                    'type': 'message',
                    'role': 'assistant',
                ...
                },
                {
                    'type': 'message',
                    'role': 'user',
                    'content': [{'type': 'input_text', 'text': '/models'}],
                },
            ]
        }
    )
...
def test_should_not_dispatch_openclaw_worker_for_multi_agent_prompt_without_estimate():
    assert not openai.should_dispatch_openclaw_worker(
...
def test_should_not_dispatch_openclaw_worker_for_slash_heavy_without_estimate():
    assert not openai.should_dispatch_openclaw_worker('/heavy 做一次最终评审', None)


def test_heavy_switch_ignores_open_webui_task_history_mentions():
    prompt = """### Task:
Generate a concise, 3-5 word title with an emoji summarizing the chat history.
### Chat History:
<chat_history>
Assistant: 需要深度推理时可切换到 lmstudio/lmstudio-community/qwen3.5-122b-a10b
</chat_history>"""

    assert not openai.looks_like_openclaw_heavy_model_switch(prompt)


def test_heavy_switch_uses_current_user_message_only():
    prompt = """[Chat messages since your last reply - for context]
Assistant: 需要深度推理时可切换到 lmstudio/lmstudio-community/qwen3.5-122b-a10b

[Current message - respond to this]
User: /122b 做一次最终评审"""

    assert openai.looks_like_openclaw_heavy_model_switch(prompt)


def test_openclaw_background_task_response_skips_title_model_call():
    result = openai.openclaw_background_task_response(
        {'model': 'openclaw/main'},
        {'task': 'title_generation'},
    )

    assert result['choices'][0]['message']['content'] == '{"title": ""}'
    assert result['model'] == 'openclaw/main'


def test_openclaw_background_task_response_ignores_normal_chat():
    result = openai.openclaw_background_task_response(
        {'model': 'openclaw/main'},
        {'task': 'chat'},
    )
...
def test_strip_openclaw_model_switch_prefix_from_responses_payload():
    payload = openai.strip_openclaw_model_switch_prefix_from_payload(
        {
            'input': [
                {
                    'type': 'message',
                    'role': 'user',
                    'content': [{'type': 'input_text', 'text': '/heavy 做一次最终评审'}],
                }
            ]
        }
    )

    assert payload['input'][0]['content'][0]['text'] == '做一次最终评审'


@pytest.mark.asyncio
async def test_resolve_openclaw_gateway_model_override_ensures_heavy_profile(monkeypatch):
    calls = []
    ...
    async def fake_fetch_openclaw_worker_json(worker_api_base_url, worker_api_token, method, path, payload=None):
        ...
        if path == '/estimate':
            return {
                'selectedProfile': 'heavy-122b',
                'selectedModel': 'lmstudio-community/qwen3.5-122b-a10b',
            }
        if path == '/profiles/ensure':
            return {'ensured': True}
    ...
    result = await openai.resolve_openclaw_gateway_model_override(
        {
            'input': [
                {
                    'type': 'message',
        ...
        {'worker_api_base_url': 'http://127.0.0.1:8090', 'worker_api_token': 'token'},
    )

    assert result == 'lmstudio/lmstudio-community/qwen3.5-122b-a10b'
    assert calls[0][1] == '/estimate'
    assert calls[1][1] == '/profiles/ensure'
    assert calls[1][2]['profile'] == 'heavy-122b'


@pytest.mark.asyncio
async def test_ensure_openclaw_gateway_default_profile_is_best_effort(monkeypatch):
    async def fake_fetch_openclaw_worker_json(worker_api_base_url, worker_api_token, method, path, payload=None):
        raise openai.HTTPException(status_code=502, detail='worker unavailable')

    monkeypatch.setattr(openai, 'fetch_openclaw_worker_json', fake_fetch_openclaw_worker_json)

    await openai.ensure_openclaw_gateway_default_profile(
        {'worker_api_base_url': 'http://127.0.0.1:8090', 'worker_api_token': 'token'}
    )
...
def test_normalize_openclaw_worker_job_payload_prefers_contextual_artifact_path(tmp_path):
    ...
    release_validation_summary = root / 'work' / 'agent-workspaces' / 'release' / 'reports' / 'validation-summary.md'
...
def test_build_openclaw_worker_artifact_response_headers_forces_download_for_binary():
    media_type, headers = openai.build_openclaw_worker_artifact_response_headers(openai.Path('/tmp/archive.zip'))
...
@pytest.mark.asyncio
async def test_openclaw_worker_job_access_allows_owner():
    payload = {
        'metadata': {
            'open_webui': {
                'user_id': 'user-1',
    ...
    await openai.ensure_openclaw_worker_job_access(payload, user)


@pytest.mark.asyncio
async def test_openclaw_worker_job_access_rejects_other_user():
    payload = {
        'metadata': {
            'open_webui': {
                'user_id': 'owner-1',
    ...
    with pytest.raises(openai.HTTPException) as exc:
        await openai.ensure_openclaw_worker_job_access(payload, user)

    assert exc.value.status_code == 403


def test_openclaw_worker_job_references_artifact_path(tmp_path):
    root = tmp_path / 'OpenClaw'
    artifact_path = root / 'work' / 'reports' / 'demo' / 'market_conclusion.md'
    other_path = root / 'work' / 'reports' / 'demo' / 'other.md'

    artifact_path.parent.mkdir(parents=True)
    (root / 'config').mkdir(parents=True)
    (root / 'config' / 'openclaw.json').write_text('{}', encoding='utf-8')
    artifact_path.write_text('# market', encoding='utf-8')
    other_path.write_text('# other', encoding='utf-8')

    job = {'resolved_artifacts': [{'label': 'market_conclusion.md', 'path': str(artifact_path.resolve())}]}

    assert openai.openclaw_worker_job_references_artifact_path(job, artifact_path.resolve())
    assert not openai.openclaw_worker_job_references_artifact_path(job, other_path.resolve())
...
@pytest.mark.asyncio
async def test_maybe_dispatch_openclaw_worker_leaves_chat_payload_on_stream(monkeypatch):
    calls = []

    async def fake_fetch_openclaw_worker_json(worker_api_base_url, worker_api_token, method, path, payload=None):
        calls.append((method, path, payload))
        raise AssertionError(f'text-only prompts should not call worker dispatch path: {path}')
...
@pytest.mark.asyncio
async def test_maybe_dispatch_openclaw_worker_does_not_inherit_prior_slash_heavy(monkeypatch):
    captured_estimate_payloads = []

    async def fake_fetch_openclaw_worker_json(worker_api_base_url, worker_api_token, method, path, payload=None):
        if path == '/estimate':
            captured_estimate_payloads.append(payload)
            return {
                'selectedAgent': 'main',
                'selectedProfile': 'main-fast',
                'selectedModel': 'qwen/qwen3.6-35b-a3b',
                'isLongTask': False,
            }

        if path == '/jobs':
            raise AssertionError('ordinary latest message should not dispatch a worker job')
    ...
    assert result is None
    assert captured_estimate_payloads == []


@pytest.mark.asyncio
async def test_maybe_dispatch_openclaw_worker_leaves_latest_slash_heavy_on_stream(monkeypatch):
    calls = []

    async def fake_fetch_openclaw_worker_json(worker_api_base_url, worker_api_token, method, path, payload=None):
        calls.append((method, path, payload))
        raise AssertionError(f'slash-heavy should not call worker dispatch path: {path}')
            ...
            'input': [
                {
                    'type': 'message',
                    'role': 'user',
                    'content': [{'type': 'input_text', 'text': 'ordinary older request'}],
                },
                {
                    'type': 'message',
                    'role': 'user',
...
async def test_maybe_dispatch_openclaw_worker_accepts_non_responses_connection(monkeypatch):
    ...
    assert job_payload['metadata']['open_webui'] == {
        'user_id': 'user-1',
        'chat_id': 'chat-1',
...
@pytest.mark.asyncio
async def test_maybe_dispatch_openclaw_worker_leaves_multi_agent_prompt_on_stream(monkeypatch):
    calls = []
...
async def test_maybe_dispatch_openclaw_worker_routes_image_attachment_payload(monkeypatch):
    ...
    assert (
        captured_calls['estimate']['metadata']['attachments'][0]['path']
        == '/Users/panda/open-webui/.data/uploads/test-avatar.png'
    )
...
@pytest.mark.asyncio
async def test_maybe_dispatch_openclaw_worker_keeps_uploaded_spreadsheet_attachment_on_stream(monkeypatch):
    captured_calls = {}
    ...
    async def fake_fetch_openclaw_worker_json(worker_api_base_url, worker_api_token, method, path, payload=None):
        captured_calls[path] = payload
        raise AssertionError(f'non-visual attachments should not call worker dispatch path: {path}')
